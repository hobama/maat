...
Expr es_constant_folding(Expr e){
    ...
    if( e->is_binop() && e->args[0]->is_cst() &&
        e->args[1]->is_cst()){
        was_tainted = e->args[0]->is_tainted() || e->args[1]->is_tainted();
        /* Binary operators */
        res = exprcst(e->size, e->concretize());
    }else if( e->is_unop() && e->args[0]->is_cst()){
        /* Unary operators */
        was_tainted = e->is_tainted();
        res = exprcst(e->size, e->concretize());
    }else if( e->is_bisz() && e->args[0]->is_cst()){
        was_tainted = e->is_tainted();
        /* BISZ */
        res = exprcst(e->size, e->args[0]->concretize() == 0 ? e->mode() : (e->mode()^1));
    }else if(   e->is_extract() && e->args[0]->is_cst() &&
                e->args[1]->is_cst() && e->args[2]->is_cst()){
        was_tainted = e->args[0]->is_tainted();
        /* Extract */ 
        res = exprcst(e->size, e->concretize());
